...
5.)  Define the sort type for the list of professionals that work on a movie.								5 [movie­id ][sort­type]
	 The sort type can be 1, 2, or 3. 1 = by ID number, 2 = by age, 3 = by number of movies that the
	 professional is listed in. The default sort type is 1, meaning that if this command is not used, the
	 movie’s professionals will be sorted by ID number.
6.)  Print the professionals of a movie.																	6 [movie­id]
7.)  Print movie details.																					7 [movie­id]
8.)  Combine movies.																						8 [movie­id],...,[movie­id]
	 This command takes at least two movie ID’s and combines all of the given movies together. Exact
	 details of how this works can be found in the program comments. The ID of the new resulting movie is
	 the concatenation of all of the ID’s of the combining movies.
9.)  Print all of the movies that a given professional works on.      										9 [pro-­id]
10.) Delete a movie.																						10 [movie­id]
11.) Delete a professional.																					11 [professional­id]
12.) Remove a professional from a specific movie (professional will still exist in the general library).	12 [movie­id] [professional­id]
